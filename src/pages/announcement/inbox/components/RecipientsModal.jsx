...
export function RecipientsModal({ open, onClose, recipients = [] }) {
  ...
  const normalizedRecipients = useMemo(() => {
    ...
    return recipients.map((recipient, index) => {
      if (typeof recipient === "string") {
        return { id: index, name: recipient, department: "", email: "" };
      }
      return {
        id:         recipient.id ?? index,
        name:       recipient.name ?? "Unknown User",
        department: recipient.department ?? recipient.dept ?? "",
  ...
  return (
    <Dialog open={open} onOpenChange={(value) => { if (!value) { onClose(); setSearch(""); } }}>
        ...
        <div className="border-b px-5 py-3">
          <div className="relative">
            ...
            <input
              ...
              className="h-10 w-full rounded-xl border border-border bg-background pl-10 pr-3 text-sm outline-none transition focus:border-blue-500 text-foreground placeholder:text-muted-foreground"
            />
          </div>
        </div>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {(recipient.department || recipient.email) && (
                      <div className="mt-1 flex flex-wrap items-center gap-1 text-[11px] text-muted-foreground">
                        ...
                        {recipient.department && recipient.email && <span>•</span>}
