...
const TYPE_BADGE = {
  ...
  other: { label: "File",  class: "bg-slate-100 text-slate-500 border border-slate-200" },
};
  ...
  return (
    <Dialog open={!!item} onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogContent className="!grid-rows-none !grid-cols-none !flex !flex-col !gap-0 !max-w-4xl !w-[90vw] !h-[90vh] !p-0 rounded-2xl overflow-hidden">
        <div className="shrink-0 flex items-center justify-between px-5 py-3 border-b bg-white">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-8 h-8 rounded-lg bg-slate-50 border flex items-center justify-center shrink-0">
              <FileIcon name={item.name} />
            </div>
            <div className="min-w-0">
              <p className="text-sm font-medium text-slate-800 truncate">{item.name}</p>
              <p className="text-[11px] text-slate-400">{item.sender} · {item.date}</p>
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <button onClick={handleDownload} className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-slate-600 hover:bg-slate-100 rounded-lg transition">
              <Download size={13} /> Download
            </button>
            <button onClick={() => window.open(url, "_blank")} className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-slate-600 hover:bg-slate-100 rounded-lg transition">
        ...
        <div className="flex-1 min-h-0 bg-slate-100 flex items-center justify-center overflow-hidden">
          {isImage ? (
  ...
  function SortHead({ label, colKey }) {
    ...
    return (
      <button
        ...
        className={"flex items-center gap-1 hover:text-slate-800 transition " + (active ? "text-slate-800 font-semibold" : "text-slate-500")}
      >
        {label}
        <ArrowUpDown size={12} className={active ? "text-blue-500" : "text-slate-300"} />
      </button>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="shrink-0 px-6 py-4 flex items-center justify-between bg-white border-b">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Attachments</h1>
          <p className="text-xs text-slate-500">All files from sent and received announcements</p>
        </div>
      ...
      {/* FILTERS */}
      <div className="shrink-0 px-6 py-3 bg-white border-b flex items-center gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-1">
          {[{ key: "all", label: "All" }, { key: "received", label: "Received" }, { key: "sent", label: "Sent" }].map((s) => (
            <button key={s.key} onClick={() => setSource(s.key)}
              className={"px-3 py-1 text-xs rounded-md transition " + (source === s.key ? "bg-white text-slate-800 shadow-sm font-medium" : "text-slate-500 hover:text-slate-700")}>
              {s.label}
            </button>
          ))}
        </div>
      ...
      {/* FILE TYPE TABS */}
      <div className="shrink-0 px-6 flex gap-0 bg-white border-b overflow-x-auto">
        {FILE_TYPE_TABS.map((t) => (
          <button key={t.key} onClick={() => setTypeTab(t.key)}
            className={"px-4 py-2.5 text-xs transition whitespace-nowrap " +
              (typeTab === t.key ? "border-b-2 border-blue-600 text-blue-600 font-medium" : "text-slate-500 hover:text-slate-700")}>
            {t.label}
          </button>
        ))}
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {filtered.map((item) => {
                ...
                return (
                  <TableRow key={item.id} onClick={() => setViewItem(item)} className="cursor-pointer hover:bg-slate-50 group">
                    <TableCell>
                    ...
                    <TableCell>
                      <span className="text-sm font-medium text-slate-800 truncate max-w-[200px] block">{item.name}</span>
                    </TableCell>
                    <TableCell>
                      <span className={"text-[10px] font-semibold px-2 py-0.5 rounded-full " + badge.class}>{badge.label}</span>
                    </TableCell>
                    <TableCell className="text-sm text-slate-600">{item.sender}</TableCell>
                    <TableCell>
                      <span className="text-xs text-slate-400 truncate max-w-[180px] block">{item.mailTitle}</span>
                    </TableCell>
                    <TableCell>
                      {item.source === "sent" ? (
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
                        <button
                          ...
                          className="p-1.5 text-slate-300 hover:text-blue-500 rounded-lg hover:bg-blue-50 transition"
                          title="Download"
                        >
                          <Download size={14} />
                        </button>
