...
import { FileText, Download, Globe, Building2, Clock, MailOpen, CornerUpRight, Eye, CheckCheck, Trash2 } from "lucide-react";
...
function SeenByIndicator({ readBy, onClick }) {
  ...
  return (
    <button onClick={onClick} className="flex items-center gap-1.5 text-[11px] text-slate-400 hover:text-slate-600 transition">
      {count > 0 ? (
        <>
  ...
  if (!selected) return (
    <div className="flex-1 flex flex-col items-center justify-center bg-slate-50 gap-4">
      ...
      <div className="text-center">
        <p className="text-sm font-medium text-slate-500">No mail selected</p>
        <p className="text-xs text-slate-400 mt-1">Select a forwarded mail from the list to view it here</p>
      </div>
    </div>
  );
    ...
    <div className="flex-1 flex flex-col bg-slate-50 overflow-hidden">
      ...
      <div className="shrink-0 px-6 py-3 bg-white border-b flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 text-[11px] text-slate-500">
            ...
            Forwarded from <span className="font-medium text-slate-700">{selected.originalSender}</span>
          </div>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        <div className="px-8 py-8">
          ...
          <h2 className="text-2xl font-semibold text-slate-900 leading-tight mb-6">
            {selected.title}
          </h2>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <span className="text-sm font-semibold text-slate-900 truncate">{senderName}</span>
                  <span className="text-[11px] text-slate-400 truncate">· {senderDept}</span>
                </div>
          ...
          {/* NOTE */}
          {selected.note && (
            <div className="mt-5 px-4 py-3 bg-[#fdfbf7] border border-slate-300 rounded-lg">
              <p className="text-[11px] uppercase tracking-widest text-[#1b2a4a] font-semibold mb-2">Note</p>
              <div
                className="text-sm text-slate-500 rich-content"
          ...
          <div className="mt-6 pl-4 border-l-2 border-slate-200">
            <p className="text-[11px] uppercase tracking-widest text-slate-400 mb-3">Original Message</p>
            <div
          ...
          {selected.attachment && (
            <div className="mt-10">
              ...
              <div className="inline-flex items-center gap-3 px-4 py-3 bg-white border border-slate-200 rounded-xl hover:border-blue-300 hover:bg-blue-50 transition group cursor-pointer">
                ...
                <button className="ml-4 p-1.5 text-slate-400 group-hover:text-blue-500 transition">
                  <Download size={14} />
                </button>
              </div>
            </div>
