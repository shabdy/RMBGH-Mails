...
import { FileEdit, Trash2, MailOpen, Search, Plus } from "lucide-react";
...
export default function Drafts() {
  ...
  const handleBulkDelete = () => { selectedIds.forEach(id => deleteDraft(id)); setSelectedIds([]); setShowBulkDelete(false); };
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="shrink-0 px-6 py-4 flex items-center justify-between bg-white border-b">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Drafts</h1>
          <p className="text-xs text-slate-500">Unsent announcements saved as drafts</p>
        </div>
        ...
        <div className="w-[380px] border-r flex flex-col h-full overflow-hidden bg-white shrink-0">
            ...
            {filtered.length > 0 && (
              <div className="flex items-center justify-between px-1">
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={toggleAll} className="text-xs text-slate-500 hover:text-slate-700">
          ...
          <div className="flex-1 min-h-0 overflow-y-auto">
            {filtered.length === 0 ? (
              ...
              filtered.map(d => {
                const preview = d.content ? d.content.replace(/<[^>]*>/g, "").slice(0, 50) || "No content" : "No content";
                ...
                return (
                  <button key={d.id} onClick={() => setSelected(d)}
                    className={"w-full text-left px-3 py-2.5 border-b hover:bg-slate-50 transition " + (selected?.id === d.id ? "bg-blue-50 border-l-2 border-l-blue-600" : "")}>
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-1">
                          <div className="flex items-center gap-1.5">
                            ...
                            <p className="text-sm font-medium truncate text-slate-800">{d.title}</p>
                          </div>
                          <span className="text-[10px] text-slate-400 shrink-0">{d.time}</span>
                        </div>
                        <p className="text-xs text-slate-400 truncate mt-0.5">{preview}</p>
                        {recipientName && <p className="text-[10px] text-slate-400 truncate mt-0.5">To: {recipientName}</p>}
                      </div>
                    </div>
                  </button>
                );
