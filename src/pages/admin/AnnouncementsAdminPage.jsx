...
const TYPE_CONFIG = {
  all:        { label: "Broadcast",  icon: Globe,      color: "bg-green-100 text-green-700 border-green-200" },
  department: { label: "Department", icon: Building2,  color: "bg-blue-100 text-blue-700 border-blue-200" },
  specific:   { label: "Direct",     icon: User,       color: "bg-purple-100 text-purple-700 border-purple-200" },
};
...
export default function AnnouncementsAdminPage() {
  ...
  useEffect(() => {
    if (!user?.id) return;
    api.get(`/admin/mail?userId=${user.id}`)
      .then(res => { setMails(res.data); setLoading(false); })
  ...
  return (
    <div className="p-6 space-y-5 overflow-y-auto h-full">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground">All announcements and mails across the organization</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          { label: "Broadcasts",  value: stats.all,    color: "bg-green-100 text-green-600" },
          { label: "Dept-wide",   value: stats.dept,   color: "bg-blue-100 text-blue-600"   },
          { label: "Direct Mail", value: stats.direct, color: "bg-purple-100 text-purple-600"},
        ].map(s => (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  <th className="px-5 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide">Sender</th>
                  ...
                  <th className="px-5 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide">Read by</th>
              ...
              <tbody className="divide-y divide-border">
                {paginated.map(m => {
                  ...
                  const cfg        = TYPE_CONFIG[m.recipientType] || TYPE_CONFIG.specific;
                  const TypeIcon   = cfg.icon;
                  const recipientLabel =
                    m.recipientType === "all"        ? "All employees" :
                    m.recipientType === "department" ? (m.from?.department || m.targetDepartmentId || "Department") :
                  ...
                  return (
                    ...
                    >
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div className="min-w-0">
                            <p className="font-medium text-foreground text-xs truncate">{senderName}</p>
                            <p className="text-[10px] text-muted-foreground truncate">{senderDept}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-3.5">
                        <p className="text-xs font-medium text-foreground truncate max-w-[200px]">{m.title}</p>
                        {m.emailTypeLabel && (
                          <span className="text-[10px] text-muted-foreground">{m.emailTypeLabel}</span>
                        )}
                      </td>
                      <td className="px-5 py-3.5">
                      ...
                      <td className="px-5 py-3.5 text-xs text-muted-foreground">{recipientLabel}</td>
                      <td className="px-5 py-3.5 text-xs text-muted-foreground">{(m.readBy || []).length}</td>
                      <td className="px-5 py-3.5">
                        <p className="text-xs text-foreground">{formatDate(m.date)}</p>
                        <p className="text-[10px] text-muted-foreground">{formatTime(m.date)}</p>
                      </td>
                    </tr>
                  );
                })}
        ...
        {selected && (
          <div className="border-t border-border bg-muted/20 px-6 py-5">
            <div className="flex items-start justify-between mb-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  From {selected.from?.name || selected.sender} · {formatDate(selected.date)} at {formatTime(selected.date)}
                </p>
              </div>
              <button
            ...
            <div
              ...
              dangerouslySetInnerHTML={{ __html: selected.content || "<em>No content</em>" }}
            />
            {selected.attachment && typeof selected.attachment === "string" && (
              <p className="text-xs text-muted-foreground mt-3">📎 {selected.attachment}</p>
            )}
          </div>
        )}
      </div>
